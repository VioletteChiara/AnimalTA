        ...
        for i in range(len(self.list_vid)):
            if (type=="Tracking" and self.list_vid[i].Track[0]) or (type=="Analyses" and self.list_vid[i].Tracked):
                ...
                if self.list_vid[i].Tracked:
                    self.Liste.itemconfig(i, {'fg': 'red'})
    ...
    def validate(self):
        ...
        if self.type=="Tracking":
            for V in list_item:
                ...
                #try:
                Do_the_track3.Do_tracking(self, Vid=self.list_vid_minus[V], folder=self.boss.folder)
        ...
        if self.type=="Analyses":
            try:
                ...
                with open(self.list_vid_minus[0].Folder+str("/Results/Results_by_ind.csv"), 'w', newline='') as file:
                    ...
                    writer.writerow(["Video", "Arena", "Individual", "Prop_time_lost", "Smoothing_filter_window","Smoothing_Polyorder", "Moving_threshold", "Prop_time_moving", "Average_Speed", "Average_Speed_Moving", "Traveled_Dist", "Traveled_Dist_Moving","Exploration_value","Exploration_method","Exploration_area","Exploration_aspect_param"])
                    pos=0
                    for V in list_item:
                        ...
                        for Area in range(len(self.Vid.Analyses[1])):#Pour chaque arène
                            ...
                            if self.Vid.Track[1][6][Area] > 1:#Dans un premier temps on regarde la distance inter-individualle (sauvegardée dans "rows_inter_dists")
                                Pts_coos = []
                                for fish in range(self.Vid.Track[1][6][Area]):
                                    Pts_coos.append(self.Coos["Ind" + str(fish + Passed_fish)])
                            ...
                            for I in range(self.Vid.Track[1][6][Area]):#On regarde ensuite les caractéristiques individuelles
                                ...
                                if self.Vid.Analyses[2][0]==0:#Si c'est method moderne
                                    ...
                                    for pt in self.Coos[ID]:
                                        if pt[0] != "NA":
                                            cv2.circle(empty, (int(pt[0]), int(pt[1])),
                                    ...
                                    new_row.append(self.Vid.Analyses[2][0])#Method
                                    new_row.append("Squares_mesh")#Area
                                    new_row.append("NA")#Param_aspect
                ...
                messagebox.showinfo(message=self.Messages["Do_anaWarn1"].format(self.list_vid_minus[V].Name, e),title=self.Messages["Do_anaWarnT1"])
