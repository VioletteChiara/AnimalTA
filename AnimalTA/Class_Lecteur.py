...
import numpy as np
import cv2
from AnimalTA import Class_Scroll_crop, UserMessages, Function_draw_mask as Dr
import time
import scipy
...
class Lecteur(Frame):
    ...
    def play(self, select=False, remove_select=False, begin=0):
        ...
        while self.playing and within:######NEW
            ...
            self.move1(nb_fr=self.jump_image)# We advance from nb_fr frames, this allow to display the video at high speed by omitting frames to go faster
            self.update()
    ...
    def update_image(self, frame, first=False, move=True):
        ...
        move=a special feature that is not used in AnimalTA, always True.
        ...
        if first:#If it is the first time the reader display a frame
            ...
            self.Prem_image_to_show = self.capture[int(frame * self.one_every) - self.Vid.Fusion[Which_part][0]].asnumpy()
            ...
            TMP_image_to_show = self.capture[int(frame * self.one_every) - self.Vid.Fusion[Which_part][0]].asnumpy()
            self.parent.modif_image(TMP_image_to_show, move=move)
