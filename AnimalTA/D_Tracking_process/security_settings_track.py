...
def check_memory_overload():
    ...
    if psutil.virtual_memory()._asdict()["percent"] > 99:#Too much memory of the computer is used, we trigger a security that will stop immediatly the program
        return 1
    elif psutil.virtual_memory()._asdict()["percent"] > 97.5: #The computer is reaching it's limit, it could come from a memory leak of decord, we add a security that will limity decord's memory leaks
        return 0
    else:
