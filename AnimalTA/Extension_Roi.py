...
class Light_changes(Frame):
    def __init__(self, parent, boss,main_frame, Vid, **kwargs):
        ...
        self.Frame_for_Graph.grid(row=20, column=0, columnspan=2, stick="nsew")
        ...
        hsb=Scrollbar(self.Frame_for_Graph, orient=HORIZONTAL, command=self.Graph.xview)
        ...
        self.loading_bar=Canvas(self.parent, height=10)
        self.loading_bar.create_rectangle(0, 0, 400, self.loading_bar.cget("height"), fill="red")
        self.loading_bar.grid(row=2, column=1)
    ...
    def modif_image(self, img, aff=True, move=True):
        new_img=np.copy(img)
        self.last_empty = img
        ...
        if aff:
            ...
            for pt in self.pts:
                new_img=cv2.circle(new_img,(int(pt[0]),int(pt[1])),5,(255,0,0),-1)
            cv2.circle(new_img,(int((self.pts[0][0]+self.pts[1][0])/2), int((self.pts[0][1]+self.pts[1][1])/2)),int(math.sqrt((self.pts[0][0]-self.pts[1][0])**2 + (self.pts[0][1]-self.pts[1][1])**2)/2),(255,0,0),2)
        ...
        if move:
            try:
                self.Graph.xview_moveto(str((self.Scrollbar.active_pos-self.multiple_width)/self.Vid.Frame_nb[1]))
            except:
                pass
        ...
        if int(self.Scrollbar.active_pos) >= round(((self.Vid.Cropped[1][0]) / self.Vid_Lecteur.one_every)) and int(
            ...
            for ind in range(len(self.pts)):
                ...
                if center[0] != "NA":
                    ...
                    if dist_clic < max(2,(5*self.Vid.Scale[0])):
    ...
    def draw_graph(self,Xs,Ys):
        ...
        for text in np.arange(200, ((self.Vid.Cropped[1][1]-self.Vid.Cropped[1][0])/self.one_every)*self.multiple_width, 200):
            self.Graph.create_text(text,self.Height-(self.ecart/2),text=round(((int((text)/self.Ratio_Xs) + int(round((self.Vid.Cropped[1][0])/self.one_every)))/self.Vid.Frame_rate[1]),2))
        ...
        cur_pos = self.Scrollbar.active_pos - int(round((self.Vid.Cropped[1][0]) / self.one_every))+1
        ...
        if self.N_lim:
            ...
            if self.selected_lim==None:
                ...
                if len(to_del)>0:
                    ...
                    self.Scrollbar.active_pos = int((x) / self.Ratio_Xs) + int(round((self.Vid.Cropped[1][0]) / self.one_every))
                    self.Scrollbar.refresh()
                    self.Vid_Lecteur.update_image(int((x) / self.Ratio_Xs) + int(round((self.Vid.Cropped[1][0]) / self.one_every)), move=False)
    ...
    def calculate_pps(self):
        if len(self.limits) == 0 or self.limits[len(self.limits)-1][1]!="NA":
            ...
            for section in self.limits:
                all_pts=[L for L in all_pts if L<min(section) or L>max(section)]
                filtered_L=[L for L in filtered_L if L<min(section) or L>max(section)]
