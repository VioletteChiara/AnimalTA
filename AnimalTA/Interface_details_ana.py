from tkinter import *
from AnimalTA import UserMessages, Class_Shapes_rows,Interface_border_portion, Function_draw_mask, Interface_extend_analyses, User_help
import numpy as np
import PIL
import math
import cv2
    ...
    def callback(self, event):
        ...
        if self.under_mouse==None and self.add_pt[0]=="Point":#If the user wanted to add a point
            ...
            for pt in range(len(self.list_of_pts)): # self.list_of_pts summarize the corners of the arena
                ...
                if self.Touched_seg((PtX, PtY), [self.list_of_pts[pt],self.list_of_pts[pt-1]]):#If not, we check if the point is touching a contour border
