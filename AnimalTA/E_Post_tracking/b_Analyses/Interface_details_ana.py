...
class Details_explo(Frame):
    ...
    def open_heatmap(self, heatmap_raw):
        self.main.Vid_Lecteur.unbindings()
        selected=list(self.List_inds_names.values()).index(self.Ind_name.get())
        ...
        list_possibilities = ['viridis', 'plasma', 'inferno', 'magma', 'cividis','Greys','Purples','Blues','Greens','Oranges']
        palette=IntVar()
        palette.set(0)
        ...
        colorbar_ymax.set(heatmap_raw.shape[0]/self.main.Vid.Scale[0])
        ...
        colorbar_xmax.set(heatmap_raw.shape[1]/self.main.Vid.Scale[0])
        ...
        def redo_vmax():
            if repre.get()==0:
                colorbar_colmax.set(round(np.max(heatmap_raw)/self.main.Vid.Frame_rate[1],3))
            else:
                colorbar_colmax.set(round(np.max(heatmap_raw) / len(self.main.Coos[0]),3))
        ...
        def check_val(var,max):
            val = float(var.get())
            if int(val * self.main.Vid.Scale[0]) > max:
                var.set(max)
                raise Exception("Can't be bigger than the actual frame")
            if int(val * self.main.Vid.Scale[0]) < 0:
                var.set(0)
        ...
        def change_style(heatmap_raw):
            ...
            try:
                ...
                heatmap_raw = heatmap_raw[
                              (heatmap_raw.shape[0] - int(ymax * self.main.Vid.Scale[0])):(
                                      heatmap_raw.shape[0] - int(ymin * self.main.Vid.Scale[0])),
                              int(xmin * self.main.Vid.Scale[0]):int(xmax * self.main.Vid.Scale[0])
                              ]
            except:
                pass

            ratio=heatmap_raw.shape[1]/heatmap_raw.shape[0]
            fig, ax = plt.subplots(figsize=(10+2, 10/ratio))  # Increase figure size to provide more space
            if reversed.get():
            ...
            try:
                ...
                vmin = 0
                if repre.get() == 0:
                    vmax = round(np.max(heatmap_raw) / self.main.Vid.Frame_rate[1], 3)
                else:
                    vmax = round(np.max(heatmap_raw) / len(self.main.Coos[0]), 3)

            im = ax.imshow(
                heatmap_raw, cmap=color, interpolation='nearest',
                extent=[0, heatmap_raw.shape[1] / self.main.Vid.Scale[0], 0,
                        heatmap_raw.shape[0] / self.main.Vid.Scale[0]],
                vmin=vmin, vmax=vmax
            )
            ...
            try:
                ...
                if xrange == 0:
                    ...
                    xticks = list(np.arange(0, heatmap_raw.shape[1] / self.main.Vid.Scale[0], xrange))
                    if len(xticks) < 50:
            ...
            try:
                ...
                if yrange == 0:
                    ...
                    yticks = list(np.arange(0, heatmap_raw.shape[0] / self.main.Vid.Scale[0], yrange))
                    if len(yticks) < 50:
            ...
            cax = fig.add_axes([pos.x1 + colorbar_pad, pos.y0 + colorbar_ypad, colorbar_width, pos.height - (2*colorbar_ypad)])  # Adjust position and size
            ...
            try:
                ...
                cbar_ticks = np.arange(vmin, vmax, rangecol)
        ...
        Radiobutton(Units_frame, text=self.Messages["Heatmap3"], variable=repre, value=0, command=partial(change_new_style, heatmap_raw), **Color_settings.My_colors.Radiobutton_Base).grid(row=cur_row_in,column=0,sticky="w")
        Radiobutton(Units_frame, text=self.Messages["Heatmap4"], variable=repre, value=1,command=partial(change_new_style, heatmap_raw), **Color_settings.My_colors.Radiobutton_Base).grid(row=cur_row_in,column=1,sticky="w")
        cur_row_in+=1
        Label(Units_frame, text=self.Messages["Heatmap5"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Units_frame, textvariable=colorbar_label, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_label.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Units_frame, text=self.Messages["Heatmap10"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Units_frame, textvariable=colorbar_xlab, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_xlab.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Units_frame, text=self.Messages["Heatmap11"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Units_frame, textvariable=colorbar_ylab, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        ...
        Label(Range_frame, text=self.Messages["Heatmap12"], font='Helvetica 9 bold', **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0, columnspan=2,sticky="ew")
        cur_row_in += 1

        Label(Range_frame, text=self.Messages["Param15"] + ":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in, column=0, sticky="ew")
        Entry(Range_frame, textvariable=colorbar_colmin, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_colmin.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Range_frame, text=self.Messages["Param16"] + ":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in, column=0, sticky="ew")
        Entry(Range_frame, textvariable=colorbar_colmax, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_colmax.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Range_frame, text=self.Messages["Heatmap13"] + ":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in, column=0,sticky="ew")
        Entry(Range_frame, textvariable=colorbar_colrange, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in, column=1,sticky="ew")
        ...
        #The axis ranges
        Label(Canvas_optn, text=self.Messages["Heatmap9"], font='Helvetica 12 bold', **Color_settings.My_colors.Label_Base).grid(row=cur_row,column=0, columnspan=2,sticky="ew")
        cur_row += 1
        ...
        Label(Xaxis_frame, text="X", font='Helvetica 10 bold', **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0, columnspan=2,sticky="ew")
        cur_row_in += 1

        Label(Xaxis_frame, text=self.Messages["Param15"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Xaxis_frame, textvariable=colorbar_xmin, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_xmin.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Xaxis_frame, text=self.Messages["Param16"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Xaxis_frame, textvariable=colorbar_xmax, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        colorbar_xmax.trace("w", partial(change_new_style, heatmap_raw))
        cur_row_in += 1

        Label(Xaxis_frame, text=self.Messages["Heatmap13"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Xaxis_frame, textvariable=colorbar_xrange, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        ...
        Label(Yaxis_frame, text=self.Messages["Param15"] + ":", **Color_settings.My_colors.Label_Base).grid(
        ...
        Label(Yaxis_frame, text=self.Messages["Param16"] + ":", **Color_settings.My_colors.Label_Base).grid(
            row=cur_row_in, column=0, sticky="ew")
        ...
        Label(Yaxis_frame, text=self.Messages["Heatmap13"]+":", **Color_settings.My_colors.Label_Base).grid(row=cur_row_in,column=0,sticky="ew")
        Entry(Yaxis_frame, textvariable=colorbar_yrange, **Color_settings.My_colors.Entry_Base).grid(row=cur_row_in,column=1,sticky="ew")
        ...
        Label(colors_frame, text=self.Messages["Heatmap14"], font='Helvetica 9 bold', **Color_settings.My_colors.Label_Base).grid(row=0, column=0, sticky="nsw")
        Label(colors_frame, text=self.Messages["Heatmap15"], font='Helvetica 9 bold', **Color_settings.My_colors.Label_Base).grid(row=0, column=1, sticky="nsw")
        cur_row += 1
        ...
        for color in list_possibilities:
            ...
            Radiobutton(colors_frame, text=list_possibilities[v], variable=palette, value=v,
                        command=partial(change_new_style, heatmap_raw), **Color_settings.My_colors.Radiobutton_Base).grid(row=row_in, column=column,sticky="w")
        ...
        Label(Canvas_optn, text=self.Messages["Heatmap16"], font='Helvetica 12 bold', **Color_settings.My_colors.Label_Base).grid(row=cur_row,column=0, columnspan=2,sticky="ew")
        cur_row += 1

        Scale(Canvas_optn, variable=cex, from_=0.05, to=2, orient=HORIZONTAL, resolution=0.05, show=True, **Color_settings.My_colors.Scale_Base).grid(row=cur_row, columnspan=2, column=0, sticky="ew")
