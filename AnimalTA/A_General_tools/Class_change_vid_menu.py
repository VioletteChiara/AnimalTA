...
class Change_Vid_Menu(Frame):
    def __init__(self, parent, main_frame, Vid, func):
        Frame.__init__(self, parent, bd=2, highlightthickness=1, relief='flat')
        ...
        if self.func!="analysis" and self.func!="check":
        ...
        for V in self.main_frame.liste_of_videos:
            if V==self.dict_Names[vid] and V!=self.Vid:
                ...
                self.main_frame.list_projects.append(
                    Class_Row_Videos.Row_Can(parent=self.main_frame.Visualise_vids, main_boss=self.main_frame, Video_file=V, proj_pos=len(self.main_frame.list_projects)))
                if self.func=="crop":
